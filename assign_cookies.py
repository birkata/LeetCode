...
class Solution:
    def findContentChildren(self, g: List[int], s: List[int]) -> int:
        # Sorting solves the simple problems first - children that are less greedy
        g.sort()
        s.sort()
        # Tracking the index and at the same time storing the count
        child = 0
        cookie = 0

        # Using AND will break the loop when the shorter list is exhausted
...
if __name__ == "__main__":
    ...
    print(sol_instance.findContentChildren(g = [1,4,8], s = [6,9]))
